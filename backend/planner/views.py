...
def get_recommend_graph():
    return create_recommend_graph()


@lru_cache(maxsize=1)
def get_evidence_coach_graph():
    return create_evidence_coach_graph()
...
class GeneratePlanView(APIView):
    ...
    def post(self, request):
        try:
            ...
            user_id = (
                getattr(user, "user_id", None) or getattr(user, "id", None) or str(user)
            )
            latest_growth = (
            ...
            result = get_recommend_graph().invoke(inputs)
            ...
            with transaction.atomic():
                ...
                plan = StudyPlan.objects.create(
                    user=user, topic=plan_topic, duration=duration
                )
                ...
                for i in range(duration):
                    ...
                    item = (
                        final_schedule[i]
                        if i < len(final_schedule) and final_schedule[i]
                        else None
                    )
            ...
            logger.exception("GeneratePlanView failed: %s", exc)
            return Response(
...
class LatestStudyPlanView(APIView):
    ...
    def get(self, request):
        ...
        plan = (
            StudyPlan.objects.filter(user=user).order_by("-created_at", "-id").first()
        )
        if not plan:
            return Response(
                {"error": "저장된 커리큘럼이 없습니다."},
                status=status.HTTP_404_NOT_FOUND,
            )

        tasks = DailyTask.objects.filter(plan=plan).order_by("day")
        base_date = (
            timezone.localdate(plan.created_at) if plan.created_at else date.today()
        )
...
class StudyTaskReflectionView(APIView):
    ...
    def patch(self, request):
        ...
        if task_id is None:
            return Response(
                {"error": "task_id는 필수입니다."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not task:
            return Response(
                {"error": "작업을 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        coach_result = {}
        try:
            coach_result = get_evidence_coach_graph().invoke(coach_input)
            logger.info("coach input=%s output=%s", coach_input, coach_result)
        except Exception as exc:
            logger.exception("StudyTaskReflectionView coach failed: %s", exc)
